...
export const generateArticle = async (req, res) => {
  try {
    ...
    const userID = auth.userID || auth.userId || auth.sub; // Adjust based on log
    if (!userID) {
      return res
        .status(401)
  ...
  try {
    ...
    if (plan !== "premium") {
      return res.json({
        success: false,
        message:
          "Limit reached. This Feature is only availavle for premium subscriptions",
      });
    }
...
export const resumeReview = async (req, res) => {
  try {
    ...
    feedback on its strength,weakness,and areas for
